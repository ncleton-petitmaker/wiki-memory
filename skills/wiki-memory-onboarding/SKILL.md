...
`Agent/` contains the public Wiki Memory plugin files. `Mémoire/` is the only target for the generated memory root and all its vaults. Never place the memory inside `Agent/`, or the agent inside `Mémoire/`. Runtimes, Python or Node environments, models, indexes, browser profiles, cookies, credentials, and caches remain in the operating system's user-data directory. Canonical evidence and the ledger are user data; Syncthing transports immutable blobs and packs from `.wiki-memory/data` but never its live SQLite or outbox.
...
Set `sync_enabled` to `true` in the confirmed onboarding spec. After `wiki-memory init` creates `<installation-root>/Mémoire` and its vaults, run `wiki-memory syncthing-setup <installation-root>/Mémoire` on the first device. This must register `Agent/` and `Mémoire/.wiki-memory/data/` as two separate Syncthing folders; never share the whole active memory database. Once the user supplies the other device's Syncthing ID, rerun with `--device-id <id>` and an optional `--device-name`. On the other device, initialize its `Mémoire/`, map the transport share to `.wiki-memory/data/`, wait for blobs and packs, import the packs, and run `wiki-memory verify` plus `rebuild`. Do not claim completion until both shares are current and import verification succeeds. Preserve the generated transport `.stignore` and confirm a separate backup/versioning policy.
...
Create a temporary onboarding spec matching `schemas/onboarding.schema.json`, prepare the two-folder installation layout, run `wiki-memory init` against its `Mémoire/` directory, then run `wiki-memory doctor`. The final doctor report must include Python, Node.js, Obsidian, Docling, QMD, canonical-ledger integrity, layout, and backup readiness. When synchronization is enabled, include Syncthing plus ignore-rule verification for `Agent/`, `Mémoire/`, and the immutable transport directory. If the target is non-empty, stop rather than merging implicitly. Offer the router and migration workflow for an existing memory.
